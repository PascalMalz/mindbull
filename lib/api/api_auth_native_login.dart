...
class ApiAuthNativeLogin {
  ...
  Future<Map<String, dynamic>> loginUser(String email, String password) async {
    const String apiUrl =
        'https://neurotune.de/sum/api/login/'; // Replace with your API endpoint
    ...
    try {
      ...
      if (response.statusCode == 200) {
        print(
            'ApiAuthNativeLogin: logged in successful with ${response.data.toString()}');
        ...
        throw DioException(
          requestOptions:
              RequestOptions(path: apiUrl), // Provide the RequestOptions
          response: response,
          error:
              'ApiAuthNativeLogin: Backend request failed with status code ${response.statusCode}',
        );
      }
    } catch (error) {
