...
class ApiFollowUser {
  ...
  Future<bool> checkFollowStatus(String userId) async {
    final String authToken = await _tokenApiKeeper.getAccessToken();
    final String apiUrl =
        'https://neurotune.de/sum/api/user/$userId/check_follow/';
    print('Follow Check');
  ...
  Future<List<dynamic>> fetchFollowersList(String userId) async {
    final String authToken = await _tokenApiKeeper.getAccessToken();
    final String apiUrl =
        'https://neurotune.de/sum/api/user/$userId/followers_list/';
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception(
            'Failed to fetch followers list. Status code: ${response.statusCode}');
      }
    } catch (error) {
  ...
  Future<List<dynamic>> fetchIFollowList(String userId) async {
    final String authToken = await _tokenApiKeeper.getAccessToken();
    final String apiUrl =
        'https://neurotune.de/sum/api/user/$userId/i_follow_list/';
    try {
      final Response response = await _dio.get(apiUrl,
          options: Options(headers: {'Authorization': 'Bearer $authToken'}));
      if (response.statusCode == 200) {
        ...
        throw Exception(
            'Failed to fetch following list. Status code: ${response.statusCode}');
      }
    } catch (error) {
