...
class ApiAuthSocialLoginAndRegistration {
  ...
  Future<bool> handleSocialLogin() async {
    ...
    if (googleUser != null) {
      ...
      String userExists = await ApiAuthCheckEmail()
          .checkEmailAvailability(email); //need the username from here
      ...
      if (userExists == 'Email already registered, please try another one.') {
        ...
        print(
            'ApiAuthSocialLoginAndRegistration handleSignIn idToken: $idToken');
        print(
            'ApiAuthSocialLoginAndRegistration handleSignIn accessToken: $accessToken');

        await _sendAccessTokenToBackend(accessToken, idToken, username);
        print(
            'ApiAuthSocialLoginAndRegistration handleSignIn Signed in with Google: ${googleUser.displayName}');
        ...
        print(
            'ApiAuthSocialLoginAndRegistration handleSignIn Google Sign-In canceled or failed.');
        return false;
      }
    } else {
      return false;
  ...
  Future<bool> handleSignIn(String? provider, String username) async {
    ...
    try {
      switch (provider) {
        case 'google':
          print(
              'ApiAuthSocialLoginAndRegistration handleSignIn provider = google');
        ...
        default:
          ...
          print(
              'ApiAuthSocialLoginAndRegistration handleSignIn Unknown provider');
          return false;
      }
    } catch (error) {
      print('ApiAuthSocialLoginAndRegistration handleSignIn error');
      print(
          'ApiAuthSocialLoginAndRegistration handleSignIn Error signing in: $error');
      return false;
    }
  }

  Future<void> _sendAccessTokenToBackend(
      String accessToken, String idToken, String username) async {
    const String apiUrl = 'https://neurotune.de/sum/auth/social/';
    ...
    print(
        '_sendAccessTokenToBackend google accessToken: $accessToken , idToken: $idToken');
    print(
        '_sendAccessTokenToBackend requestBody: $requestBody ,headers: $headers');
    try {
      ...
      if (response.statusCode == 200) {
        ...
        print(
            'ApiAuthSocialLoginAndRegistration _sendAccessTokenToBackend: user authenticated with social login refreshToken: $refreshToken , accessToke: $accessToken');
        ...
        print(
            'ApiAuthNativeLogin: User authenticated with native login. accessToken: $accessToken, refreshToken: $refreshToken ');
  ...
  Future<void> handleSignOut() async {
    try {
      ...
      print(
          'ApiAuthSocialLoginAndRegistration handleSignOut Signed out from Google');
    } catch (error) {
      print(
          'ApiAuthSocialLoginAndRegistration handleSignOut Error signing out: $error');
    }
/*    setState(() {
      username='';
    });*/
