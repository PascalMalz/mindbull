...
class ApiPostPost {
  ...
  Future<String?> uploadPost({
    ...
    List<String>? tags,
    Function(int, int)?
        onUploadProgress, // Added a callback function for upload progress
  }) async {
    const String apiUrl =
        'https://neurotune.de/api/post_content/'; // Replace with your actual API endpoint
    ...
    if (composition != null) {
      ...
      Set<AudioFileDetails> audioDetails =
          extractAudioDetails(jsonEncode(composition.toJson()));
      ...
      for (AudioFileDetails details in audioDetails) {
        formData.files.add(
          MapEntry(
            ...
            await MultipartFile.fromFile(details.filePath,
      ...
      formData.fields
          .add(MapEntry("composition", jsonEncode(composition.toJson())));
    }

    // Add other post details to FormData
    formData.fields
      ..add(MapEntry("description", description ?? ""))
      ..add(MapEntry("user", userDataProvider.currentUser?.id ?? ''))
      ..add(MapEntry("tags", tags?.join(',') ?? ""));
    ...
    for (var field in formData.fields) {
      ...
      if (field.value.length > chunkSize) {
        for (int i = 0; i < field.value.length; i += chunkSize) {
          print(
              "\tValue chunk: ${field.value.substring(i, i + chunkSize > field.value.length ? field.value.length : i + chunkSize)}");
        }
      } else {
...
Set<AudioFileDetails> extractAudioDetails(String compositionJson) {
  ...
  for (var match in matches) {
    ...
    audioDetails
        .add(AudioFileDetails(filePath, title)); // Add the extracted details
  }
  return audioDetails;
}
