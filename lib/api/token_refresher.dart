...
/*    initiateTokenRefreshTimers();*/
  ...
  void initiateTokenRefreshTimers() {
    ...
    _nextAccessTokenRefresh = DateTime.now().add(const Duration(seconds: 20));
  ...
  void startAccessTokenTimer() {
    final durationUntilNextRefresh =
        _nextAccessTokenRefresh.difference(DateTime.now());
...
Future<bool> refreshAccessAndRefreshToken(String currentRefreshToken) async {
  ...
  if (currentRefreshToken.isEmpty) {
    print(
        'TokenRefresher: refreshAccessToken: Token empty! currentRefreshToken: $currentRefreshToken');
    return false;
  }
  ...
  try {
    ...
    print(
        "TokenRefresher: refreshAccessToken: Error refreshing access token: $error, $currentRefreshToken");
    return false;
  }
}
