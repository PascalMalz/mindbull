...
import 'package:mindbull/services/log_service.dart';
...
Future<void> callbackDispatcher() async {
  ...
  Workmanager().executeTask((task, inputData) async {
    ...
    try {
      ...
      logService
          .writeLog('callbackDispatcher: Workmanager called ${DateTime.now()}');
      prefs.setInt(
      ...
      print(
          'callbackDispatcher: Workmanager failed: ${err.toString()}'); // Prints error on the debug console
      taskSuccess = false;
    }
...
Future<bool> refreshAccessAndRefreshToken(String currentRefreshToken) async {
  final Dio dio = Dio();
  const String apiUrl = 'https://neurotune.de/sum/api/token/refresh/';
  ...
  if (currentRefreshToken.isEmpty) {
    print(
        'callbackDispatcher: refreshAccessToken: Token empty! currentRefreshToken: $currentRefreshToken');
    return false;
  }
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      print(
          'callbackDispatcher:  refreshAccessToken: newRefreshToken: $newRefreshToken');
      return true;
    } else {
      return false;
    }
    ...
    print(
        "callbackDispatcher: refreshAccessToken: Error refreshing access token: $error");
    return false;
  }
}
