...
import 'package:mindbull/models/personal_growth_characteristic.dart';
...
class CharacteristicsProvider with ChangeNotifier {
  ...
  Map<String, List<PersonalGrowthCharacteristic>> get categorizedEntries =>
      _categorizedEntries;

  Future<void> _loadDataFromHive() async {
    // Load characteristics and ratings from Hive
    var characteristics = _characteristicsBox.values
        .cast<PersonalGrowthCharacteristic>()
        .toList();
  ...
  Future<void> fetchAndUpdateCharacteristics() async {
    try {
      var fetchedCharacteristics =
          await ApiCharacteristicsService.fetchCharacteristics();
      print('Fetched characteristics: $fetchedCharacteristics');

      // Load existing ratings from Hive
      var existingCharacteristics = _characteristicsBox.values
          .cast<PersonalGrowthCharacteristic>()
          .toList();
      var existingRatingsMap = {
        for (var c in existingCharacteristics) c.name: c.rating
      };
  ...
  void updateRating(String category,
      PersonalGrowthCharacteristic characteristic, int newRating) {
    ...
    List<PersonalGrowthCharacteristic> categoryList =
        _categorizedEntries[category] ?? [];
    int indexToUpdate =
        categoryList.indexWhere((c) => c.name == characteristic.name);
    if (indexToUpdate != -1) {
    ...
    print(
        'rating entry stored in Hive, name: ${characteristic.name}, characteristic.rating.value: ${characteristic.rating.value}');
