...
Future<void> getVideoMetadata(String videoUrl) async {
  final session = await FFprobeKit.getMediaInformation(videoUrl);
  ...
  if (mediaInfo != null) {
    ...
    if (streams.isNotEmpty) {
      // Safely iterate over the streams to find a video stream
      final videoStream = streams.firstWhere(
        (stream) => stream.getAllProperties()?['codec_type'] == 'video',
        orElse: () => StreamInformation({}),
      );

      // Ensure videoStream contains valid data
      final properties = videoStream.getAllProperties();
      if (properties != null) {
        final width = properties['width'];
        final height = properties['height'];
        final rotation = properties['tags']?['rotate'];

        print('getVideoMetadata Width: $width');
        print('getVideoMetadata Height: $height');
        print('getVideoMetadata Rotation: $rotation');
      } else {
        print('getVideoMetadata No valid video stream found.');
      }
    } else {
      print('getVideoMetadata No streams found.');
    }
  } else {
    print('getVideoMetadata Failed to retrieve media information.');
  }
}
...
class _VideoPlayerWidgetState extends State<VideoPlayerWidget> {
  ...
  void _initVideoPlayer() {
    ...
    getVideoMetadata(widget.videoUrl!);
                ...
                child: AspectRatio(
                  ...
                  child: Stack(
                    ...
                    children: <Widget>[
                      ...
                      _ControlsOverlay(
                          controller:
                              _controller), // Reintegrated overlay controls
                    ],
                  ),
                ),
              ),
      ...
      children: <Widget>[
        ...
        Positioned.fill(
          child: GestureDetector(
            onTap: () {
              if (!_isInteracting) {
                // Only toggle play/pause when not interacting with the slider
                if (isPlaying) {
                  widget.controller.pause();
                } else {
                  widget.controller.play();
                  _hasBeenPlayed =
                      true; // Set this when the video is played for the first time
                }
              }
            },
          ),
          ...
          Positioned(
            ...
            child: SliderTheme(
              ...
              child: Slider(
                ...
                onChangeStart: (_) {
                  _isInteracting =
                      true; // Mark that the user is interacting with the slider
                },
                onChanged: (value) {
                  setState(() {
                    _sliderValue = value;
                ...
                onChangeEnd: (value) {
                  ...
                  _isInteracting =
                      false; // Mark that the user has finished interacting with the slider
                },
