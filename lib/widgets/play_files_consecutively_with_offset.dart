...
class playFilesConsecutivelyWithOffset extends StatefulWidget {
  ...
  final Function(bool isPlaying)
      onAudioFilePlayingStateChanged; // New callback function

  const playFilesConsecutivelyWithOffset({
    super.key,
...
class _playFilesConsecutivelyWithOffsetState
    extends State<playFilesConsecutivelyWithOffset> {
  int currentIndex = 0;
  ...
  void playAudioFiles() async {
    ...
    for (var i = 0; i < widget.audioFiles.length; i++) {
      ...
      await Future.delayed(Duration(
          milliseconds: widget.audioFiles[i]['durationMilliseconds'] *
              widget.audioFiles[i]['repetition']));
    ...
    setState(() {
      isPlaying =
          false; // Set isPlaying to false when audioFiles finish playing
    });
    widget.onAudioFilePlayingStateChanged(
        isPlaying); // Notify the callback function
  }

  @override
  Widget build(BuildContext context) {
    return Text(
        'Now playing: ${widget.audioFiles[currentIndex]["title"]}'); // Display the currently playing audioFile
  }
}
