...
class CustomLoginScreen extends StatefulWidget {
  ...
  const CustomLoginScreen({super.key, this.isSignUp = false});
...
class _CustomLoginScreenState extends State<CustomLoginScreen> {
  ...
  Future<bool?> _handleSignup(
      {String? loginMethod, String? authProvider}) async {
    if (loginMethod == 'social') {
      ...
      if (existingAccount == true) {
        ...
        Navigator.pop(
            context, userDataProvider.currentUser?.id); // Close the dialog
      ...
      if (emailErrorText == null && passwordErrorText == null) {
        ...
        String emailAvailable = await ApiAuthCheckEmail()
  ...
  Widget _buildButton(String text, String? customIconPath,
        ...
        child: Row(
          ...
          children: [
            if (customIconPath != null)
              SvgPicture.asset(
                // Use SvgPicture.asset to load SVG icons
                customIconPath,
                //color: Colors.black87,
                width: 32,
                height: 32,
              ),
            if (customIconPath != null) const SizedBox(width: 16),
            Text(
              text,
                ...
                Column(
                  ...
                  children: [
                    ...
                    Column(
                      children: [
                        _buildButton(
                            'Sign in with Google', 'assets/Google__G__Logo.svg',
                            onPressed: () async {
...
/*                  SizedBox(height: 20),
                  ...
                  _buildButton('Sign in with Twitter', 'assets/twitter_logo.svg', onPressed: () async{
                    ...
                    const SizedBox(
                        height:
                            20), // Add spacing between "Create New Account" button and text
...
class _UsernameInputScreenState extends State<UsernameInputScreen> {
  ...
  Future<void> _checkUserName() async {
    ...
    if (usernameErrorText == null) {
      //Check if username exists in backend:
      String userAvailable = await ApiAuthCheckUsernameAvailability()
...
// Helper method to create clickable text
  ...
  Future<void> _showPolicyPopup(
      BuildContext context, String policyTitle) async {
  ...
  Future<void> _handleRegistration(String userName) async {
    ...
    final registrationAPI =
        getIt<ApiAuthNativeRegistration>(); // Get the registered instance
    final registrationResult =
        await registrationAPI.registerUser(email, password, username);
        ...
        builder: (context) {
          return AlertDialog(
            ...
            content: Column(
              mainAxisSize: MainAxisSize.min,
              children: [
                const Text('Please confirm your email to activate your user.'),
                const SizedBox(height: 16),
                ElevatedButton(
                  onPressed: () {
              ...
              Card(
                ...
                child: Padding(
                  ...
                  child: Column(
                    children: [
                      const Text(
                        'Please set an username. The username will be neccessary to find your profile and can be displayed for social interactions. The length must be at least 5 characters.',
                        style: TextStyle(color: Colors.greenAccent),
                      ),
                      const SizedBox(height: 20),
                      UsernameField(
...
class UsernameInputScreenSocial extends StatefulWidget {
  ...
  const UsernameInputScreenSocial({
    super.key,
    this.authenticationProvider,
  });

  @override
  _UsernameInputScreenSocialState createState() =>
      _UsernameInputScreenSocialState();
}
...
// Helper method to create clickable text
  ...
  Future<void> _handleRegistration(String userName) async {
    ...
    final socialAuth = getIt<
        ApiAuthSocialLoginAndRegistration>(); // Get the registered instance
    final registrationResult =
        await socialAuth.handleSignIn(widget.authenticationProvider, userName);
    print('registrationResult: $registrationResult');
    // Show an error message using a SnackBar
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
...
class LoginPage extends StatefulWidget {
  ...
  const LoginPage({super.key, required this.redirectRoute});
  @override
              ...
              child: TextFormField(
                ...
                decoration: InputDecoration(
                  ...
                  focusedBorder: const UnderlineInputBorder(
                    borderSide: BorderSide(
                      color:
                          Colors.greenAccent, // Replace with your desired color
                      width: 2.0, // Adjust the width of the line
                    ),
                  ),
                ),
...
class UsernameField extends BaseField {
  ...
  String? validate(String value) {
    ...
    final validCharacters = RegExp(
        r'^[a-zA-Z0-9._]+$'); // Only letters, numbers, periods, and underscores are allowed
