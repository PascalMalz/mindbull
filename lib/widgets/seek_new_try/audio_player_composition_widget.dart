...
class AudioPlayerCompositionWidget extends StatefulWidget {
  ...
  const AudioPlayerCompositionWidget(
      {super.key, required this.composition, this.autoplayEnabled = false});

  @override
  _AudioPlayerCompositionWidgetState createState() =>
      _AudioPlayerCompositionWidgetState();
}

class _AudioPlayerCompositionWidgetState
    extends State<AudioPlayerCompositionWidget> {
  late SeamlessAudioPlayerComposition _audioPlayer;
  late Stream<Duration> _positionStream =
      const Stream<Duration>.empty(); // Initialize with an empty stream

  @override
  void initState() {
    super.initState();
    _audioPlayer = SeamlessAudioPlayerComposition(widget.composition,
        autoplayEnabled: widget.autoplayEnabled);

    _audioPlayer.initialize().then((_) {
      if (mounted && widget.autoplayEnabled) {
        // Check if the widget is still in the tree and autoplay is enabled
        _audioPlayer.play();
      }
      setState(() {
