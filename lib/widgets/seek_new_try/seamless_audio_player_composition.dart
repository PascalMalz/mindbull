...
class SeamlessAudioPlayerComposition {
  ...
  final Duration _lastUpdatedPosition = Duration.zero;
  ...
  SeamlessAudioPlayerComposition(this.composition,
  ...
  Future<void> initialize() async {
    ...
    for (var compAudio in compositionAudios) {
      if (compAudio.content.durationInMilliseconds == 0) {
        ...
        _totalDuration +=
            Duration(milliseconds: compAudio.content.durationInMilliseconds) *
                compAudio.audioRepetition;
      }
    }
    ...
    if (_isDurationAvailable && autoplayEnabled) {
      ...
      await _audioPlayer
          .setUrl(compositionAudios.first.content.clientAppAudioFilePath);
    }
    // Set up a timer to periodically update the position stream
    _positionUpdateTimer =
        Timer.periodic(Duration(milliseconds: _updateFrequencyMillis), (timer) {
    ...
    _audioPlayer.playerStateStream.listen((state) {
      if (state.processingState == ProcessingState.completed) {
        ...
        if (_currentFileIndex == compositionAudios.length - 1 &&
            _currentRepetition ==
                compositionAudios[_currentFileIndex].audioRepetition - 1) {
          _positionController
              .add(_totalDuration); // Signal the end of the total playback
        } else {
  ...
  Duration _calculateCurrentPosition() {
    ...
    for (int i = 0; i < _currentFileIndex; i++) {
      cumulative += Duration(
              milliseconds:
                  compositionAudios[i].content.durationInMilliseconds) *
          compositionAudios[i].audioRepetition;
    }
    cumulative += Duration(
        milliseconds: _currentRepetition *
            compositionAudios[_currentFileIndex]
                .content
  ...
  Future<void> _playFileAtIndex(int index) async {
    ...
    await _audioPlayer
        .setUrl(compositionAudios[index].content.clientAppAudioFilePath);
  ...
  Future<void> seek(Duration position) async {
    ...
    for (var i = 0; i < compositionAudios.length; i++) {
      var fileDuration = Duration(
          milliseconds: compositionAudios[i].content.durationInMilliseconds);
      var fileTotalDuration =
          fileDuration * compositionAudios[i].audioRepetition;
      ...
      if (position < cumulative + fileTotalDuration) {
        ...
        var repetitionIndex =
            positionInCurrentFile.inMilliseconds ~/ fileDuration.inMilliseconds;
        repetitionIndex =
            min(repetitionIndex, compositionAudios[i].audioRepetition - 1);
        var positionInRepetition =
            positionInCurrentFile.inMilliseconds % fileDuration.inMilliseconds;
        ...
        if (i != _currentFileIndex) {
          ...
          await _audioPlayer
              .setUrl(compositionAudios[i].content.clientAppAudioFilePath);
        }
        ...
        var correctPosition =
            cumulative + Duration(milliseconds: positionInRepetition);
        correctPosition += Duration(
            milliseconds: repetitionIndex * fileDuration.inMilliseconds);
  ...
  Future<void> _playNextFile() async {
    print(
        "Current File Index: $_currentFileIndex, Current Repetition: $_currentRepetition");
    ...
    if (_currentRepetition <
      ...
      if (_currentFileIndex >= compositionAudios.length) {
        ...
        _currentFileIndex =
            0; // Consider what behavior you want at the end of the playlist
