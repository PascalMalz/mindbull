...
import 'package:mindbull/widgets/seek_new_try/seamless_audio_player_single_audio.dart';
...
class AudioPlayerSingleAudioWidget extends StatefulWidget {
  ...
  const AudioPlayerSingleAudioWidget(
      {super.key, required this.audioFile, this.autoplayEnabled = false});

  @override
  _AudioPlayerSingleAudioWidgetState createState() =>
      _AudioPlayerSingleAudioWidgetState();
}

class _AudioPlayerSingleAudioWidgetState
    extends State<AudioPlayerSingleAudioWidget> {
  late SeamlessAudioPlayerSingleAudio _audioPlayer;
  late Stream<Duration> _positionStream = const Stream<Duration>.empty();
  bool _isPlaying = false; // State to track play/pause status

  @override
  void initState() {
    super.initState();
    _audioPlayer = SeamlessAudioPlayerSingleAudio(widget.audioFile,
