...
import 'package:mindbull/models/composition_audio.dart';
...
class AudioFileListScreen extends StatefulWidget {
  const AudioFileListScreen({super.key, required this.title});
...
class _AudioFileListScreenState extends State<AudioFileListScreen> {
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: const Text('Choose the source of the file'),
          actions: [
            TextButton(
              onPressed: () async {
              ...
              onPressed: () async {
                ...
                if (selectedFileOrComposition is Audio) {
                  ...
                  await _getAudioDuration(
                      audioFile); // Calculate the duration for the audioFile
                  audioListProvider.addCompositionAudio(audioFile, 0,
                      1); // Adjust this method to accept CompositionAudio
                  ...
                  print(
                      "selectedFileOrComposition.id: ${selectedFileOrComposition.id}");
                  for (CompositionAudio file
                      in selectedFileOrComposition.compositionAudios) {
                    print(
                        "selectedFileOrComposition.compositionAudio: ${file.content.id}");
                  }

                  audioListProvider.addCompositionAudio(composition, 0,
                      1); // Adjust this method to accept CompositionAudio
  ...
  void _addFileToList(String filePath) async {
    ...
    if (extension == '.mp3' ||
      ...
      await _getAudioDuration(
          audioFile); // Wait for the duration to be retrieved

      setState(() {
        audioListProvider.addCompositionAudio(
            audioFile, 0, 1); // Default position and repetition
        updateAudioFileList();
      });
    }
  }
  ...
  void updateAudioFileList() {
    ...
    _audioFileList = audioListProvider.getCompositionAudios
        .map((compContent) {
  ...
  void _updateRepetition(CompositionAudio compAudio, String newValue) {
    final newRepetition = int.tryParse(newValue) ??
        compAudio.audioRepetition; // Default to current value on parse failure
    final audioListProvider =
        Provider.of<AudioListProvider>(context, listen: false);

    // Update the repetition using the existing method in the provider
    audioListProvider.updateCompositionAudio(
        compAudio.compositionAudioId, compAudio.audioPosition, newRepetition);
...
// Before the loop, ensure compAudio is also a CompositionAudio to access its id safely.
      if (compAudio is CompositionAudio) {
        for (var compContent in audioListProvider.getCompositionAudios) {
          print(
              'compContent.id: ${compContent.compositionAudioId}, compAudio.id: ${compAudio.compositionAudioId}');
          if (compContent.compositionAudioId == compAudio.compositionAudioId) {
            // If a matching CompositionAudio is found, remove it using its ID
            setState(() {
              audioListProvider
                  .removeCompositionAudio(compContent.compositionAudioId);
              updateAudioFileList();
            });
  ...
  void _playFile(String clientAppAudioFilePath) async {
    ...
    if (!_wavePlayer.playerState.isStopped) {
      ...
      setState(() {
        _isPlaying =
            _wavePlayer.playerState.isPaused; // Update the play/pause state
      });
      return;
    }
    _startNewPlayer(clientAppAudioFilePath);
    ...
    return await showDialog<bool>(
          ...
          builder: (BuildContext context) {
            return AlertDialog(
              ...
              content: const Text(
                  'Are you sure you want to remove this audioFile from the mix?'),
              actions: [
                TextButton(
  ...
  Widget build(BuildContext context) {
    ...
    const itemHeight = 90.0; // Adjust this value based on your item height
    print(screenHeight);
    final maxVisibleItems = (screenHeight - 60) ~/ itemHeight;
    bool isScrollable =
        audioListProvider.compositionAudios.length > maxVisibleItems;
    return Scaffold(
      backgroundColor: Colors.white,
      appBar: AppBar(
        backgroundColor: Colors.transparent,
        title: const Text('Mix your mind sounds'), //Text(widget.title),
        actions: [
          Row(
            ...
            children: [
              const Text('Remove all',
                  style: TextStyle(
                      color: Colors.black)), // Assuming your AppBar is dark
              IconButton(
            ...
            child: ReorderableListView(
              ...
              onReorder: (int oldIndex, int newIndex) {
                setState(() {
                  ...
                  final compContent =
                      audioListProvider.compositionAudios.removeAt(oldIndex);
                  audioListProvider.compositionAudios
                      .insert(newIndex, compContent);
                  ...
                  Padding(
                    ...
                    child: Container(
                      ...
                      child: ReorderableDragStartListener(
                        ...
                        index: audioListProvider.compositionAudios
                            .indexOf(compContent),
                        child: Transform(
                          transform: Matrix4.identity()
                            ..setEntry(3, 2, 0.001)
                            ..rotateX(0.02 *
                                audioListProvider.compositionAudios.indexOf(
                                    compContent)), // Adjust the rotation angle as desired
                          child: Container(
                            decoration: BoxDecoration(
                              borderRadius: BorderRadius.circular(10.0),
                              color: Colors.deepPurple,
                            ...
                            child: ListTile(
                              ...
                              title: Row(
                                children: [
                                        ...
                                        children: [
                                          ...
                                          SizedBox(
                                            ...
                                            child: compContent
                                                    ...
                                                    15 // Customize the condition based on your desired length
                                                ? Marquee(
                                                    text: compContent
                                                        .content.title,
                                                    style: const TextStyle(
                                          ...
                                          Text(
                                            '${compContent.content.duration} s',
                                            style:
                                  ...
                                  const SizedBox(width: 8.0),
                                  const Icon(
                                    MyFlutterApp.icon_drag_drop_list,
                                    color: Colors.white,
                                  ),
                                  const SizedBox(width: 4.0),
                                  Container(
  ...
  void _showRepetitionDialog(CompositionAudio compAudio) {
    final TextEditingController repetitionController =
        TextEditingController(text: compAudio.audioRepetition.toString());
