...
class YourLibrary extends StatefulWidget {
  const YourLibrary({super.key, required this.title});
...
class _YourLibraryState extends State<YourLibrary> {
  ...
  void _removeFileFromAllJsonFiles(String filePath) async {
    ...
    for (final jsonFile in jsonFiles) {
      ...
      final updatedData = jsonData.where((item) {
        final itemclientAppAudioFilePath =
            item['clientAppAudioFilePath'] as String?;
  ...
  Audio? _removeAudioFileFromFileList(String filePath) {
    final audioFile = _audioFileList.firstWhere(
        (audioFile) => audioFile.clientAppAudioFilePath == filePath);
            ...
            children: [
              FilterButton(
                ...
                onPressed: () => _filterByFileType(
                  _selectedFileType == FileType.mp3 ||
                          _selectedFileType == FileType.all
                      ? FileType.all
                      : FileType.mp3,
                ),
              ...
              FilterButton(
                ...
                isActive: _selectedFileType == FileType.json ||
                    _selectedFileType == FileType.all,
                onPressed: () => _filterByFileType(
                  _selectedFileType == FileType.json ||
                          _selectedFileType == FileType.all
                      ? FileType.all
                      : FileType.json,
                ),
              ...
              itemBuilder: (context, index) {
                ...
                return ListTile(
                  title: Row(
                    children: [
                      IconButton(
                        icon: _currentclientAppAudioFilePath ==
                                audioFile.clientAppAudioFilePath
                            ? const Icon(Icons.pause_circle_outline)
                            : const Icon(Icons.play_arrow),
                        onPressed: () {
                          setState(() {
                            if (_currentclientAppAudioFilePath ==
                                audioFile.clientAppAudioFilePath) {
