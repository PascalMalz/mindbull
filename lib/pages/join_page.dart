...
import 'package:mindbull/models/composition.dart';
...
// Load audio files from the provider
  void _updateAudioFileListFromProvider() {
    ...
    _audioFileList = audioListProvider.compositionAudios.map((compAudio) {
      final controller =
          TextEditingController(text: compAudio.content.description ?? "");
    ...
    _progressTimer =
      ...
      if (isDurationAvailable) {
        setState(() {
          ...
          _progress = (currentPosition.inMilliseconds.toDouble() /
  ...
  Duration? _calculateTotalDuration() {
    ...
    final totalDuration = _audioFileList.fold<Duration>(
        Duration.zero,
        (previous, audioFile) =>
            previous + Duration(milliseconds: audioFile.length));
  ...
  Future<void> _seekToPosition(double value) async {
    final totalDuration = _calculateTotalDuration();
    final position =
        (value * (totalDuration?.inMilliseconds.toDouble() ?? 0.0)).toInt();
  ...
  final List<TextEditingController> _audioDescriptionControllers = [];
  ...
  Widget build(BuildContext context) {
    final audioListProvider =
        Provider.of<AudioListProvider>(context, listen: true);
    ...
    return Scaffold(
            ...
            onPressed: () {
              ...
              if (fileName.isNotEmpty) {
                ...
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(
                      content: Text('Please enter a valid Composition name.')),
                );
              }
            },
          ),
      ...
      body: Theme(
        ...
        child: ListView(
          ...
          children: [
            const Text('Set Composition name:',
            ...
            Column(
              children: [
                ...
                const Align(
                    ...
                    child: Text('Enter Composition description:',
                        style: TextStyle(
                          color: Colors.white,
                          fontSize: 16,
                        ))),
                ...
                (tags.isNotEmpty && tags.any((tag) => tag.isNotEmpty))
                    ? Wrap(
                        children:
                            tags.where((tag) => tag.isNotEmpty).map((tag) {
                          return Padding(
            ...
            const SizedBox(height: 16.0),
            const Text('Enter Subtitles / Description for each file:',
                style: TextStyle(color: Colors.white, fontSize: 16)),
            const SizedBox(
              height: 5,
            ),
            ...compFiles.asMap().entries.map((entry) {
              ...
              return Card(
                ...
                child: Column(
                  ...
                  children: [
                    ListTile(
                      title: Text(compAudio.content.title,
                          style: const TextStyle(color: Colors.white)),
                    ...
                    Wrap(
                      children: compAudio.content.tags.map<Widget>((tag) {
                        print(
                            'wrap: $tag'); // Assume each audioFile has a tags list
                        return Padding(
                          padding: const EdgeInsets.all(4.0),
                          child: Chip(
                            backgroundColor: Colors.deepPurple,
                            ...
                            onDeleted: () {
                              audioListProvider.removeTagFromFileById(
                                  compAudio.compositionAudioId,
