...
import 'package:mindbull/pages/profile_edit_page.dart';
...
class _ProfilePageState extends State<ProfilePage> {
  ...
  void initState() {
    ...
    if (widget.userId != null &&
        widget.userId != userDataProvider.currentUser?.id) {
  ...
  Future<void> _pickImage() async {
    ...
    if (xFile != null) {
      ...
      final String? uploadResponse =
          await _apiUserProfileUpload.uploadImage(_imageFile!);
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content:
              const Text('Are you sure you want to delete this audio file?'),
          actions: [
            TextButton(
              onPressed: () async {
                try {
  ...
  Widget _buildAuthenticationAction() {
    ...
    if (user == null) {
      return IconButton(
        ...
        onPressed: () async {
          await Navigator.pushNamed(context, '/authentication',
  ...
  Widget build(BuildContext context) {
    ...
    final isLoggedIn = authProvider
        .isLoggedIn; // Assuming AuthProvider has an isLoggedIn property
    String? profileUser;
                    ...
                    children: [
                      Consumer<UserDataProvider>(
                        builder: (context, userDataProvider, child) {
                          ...
                          print(
                              'profile load userProfile pic widget.userId: ${widget.userId}');
                          // This boolean directly tells us if we're viewing our own profile or another's.
                          final bool isOwnProfile = widget.userId == null ||
                              widget.userId == userDataProvider.currentUser?.id;
                          print(
                              "profile load userProfile pic isOwnProfile: $isOwnProfile");
                          // Decide which user's profile picture URL to use.
                          String? profileUrl = isOwnProfile
                              ? userDataProvider.currentUser?.profilePictureUrl
                              : userDataProvider
                                  .getOtherUser(widget.userId!)
                                  ?.profilePictureUrl;
                          print(
                              'profile load userProfile pic profileUrl: $profileUrl');
                        ...
                        children: [
                          ...
                          Consumer<UserDataProvider>(
                            builder: (context, userDataProvider, child) {
                              ...
                              String? userName = isOwnProfile
                                  ? userDataProvider.currentUser?.username
                                  : userDataProvider
                                      .getOtherUser(widget.userId!)
                                      ?.username;
                              ...
                              if (user != null) {
                                return Column(
                                  ...
                                  children: [
                                    Text(
                                      userName ??
                                          'Default Username', // Replace with the actual user display name
                                    ...
                                    if (!isOwnProfile)
                                      Column(
                                        children: [
                                          ElevatedButton(
                                            onPressed: () async {
                                              ApiFollowUser apiFollowUser =
                                                  getIt<ApiFollowUser>();
                                              try {
                                                await apiFollowUser.followUser(
                                ...
                                return const Text(
                                  'Login to view / edit your profile',
                                  style: TextStyle(
                                    color: Colors.black,
                                  ),
                                );
                  ...
                  isLoggedIn
                      ? PostsAudioImageDisplayScreen(
                          ...
                          userId: widget.userId ??
                              currentUserId, // Pass the current or specified user's ID
