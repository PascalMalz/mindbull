  ...
  Future<void> ensureDirectoryExists(String path) async {
    ...
    if (!await directory.exists()) {
      await directory.create(
          recursive:
              true); // Using recursive: true ensures that all non-existing intermediate directories are also created.
    }
  }
...
// Usage:
  ...
  Future<void> _startRecording() async {
    _filePath =
        '${(await getApplicationDocumentsDirectory()).path}/default.aac';
...
/*      _wavePlayer.onCompletion.listen((_){    setState(() {
  ...
  void _showFilesInDirectory() async {
    ...
    if (!await directory.exists()) {
      await directory
          .create(); // This will create the directory if it doesn't exist
    }
    ...
    _files.sort((a, b) => File(b.path)
        .lastModifiedSync()
        .compareTo(File(a.path).lastModifiedSync()));
    print(
        "Sorted Files: ${_files.map((file) => File(file.path).lastModifiedSync().toString()).toList()}");
    setState(() {
      _files = allFiles;
    });
    _files.sort((a, b) {
      return b.statSync().modified.compareTo(
          a.statSync().modified); // using b first for descending order
    });
  }
  ...
  Future<bool> saveFile(BuildContext context) async {
    ...
    Audio audioFile = Audio(
      ...
      durationInMilliseconds:
          _recorderController.recordedDuration.inMilliseconds.toInt(),
      username: userName,
  ...
  Future<bool> saveOrUpdateHiveAudioMeta(Audio newAudioFile) async {
    ...
    for (var key in metadataBox.keys) {
      ...
      if (existingAudioFile != null &&
          existingAudioFile.title == newAudioFile.title) {
        matchingKey = key;
        break;
      }
    }
    ...
    for (var audioFile in metadataBox.values) {
  ...
  Future<bool> _uploadMusic(File file, String fileName) async {
    ...
    await musicApi.uploadMusic(
      ...
      onError: (String errorMessage) {
        success = false;
        print(
            errorMessage); // You can display this error message to the user if needed
      },
    );
    return success;
  }
