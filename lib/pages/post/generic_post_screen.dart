...
class GenericPostScreen extends StatefulWidget {
  const GenericPostScreen({super.key});
...
class _GenericPostScreenState extends State<GenericPostScreen> {
  ...
  void initState() {
    ...
    _audioPlayer.positionStream
        .listen((position) => setState(() => _audioPosition = position));
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Media Playback'),
            ...
            if (_videoController != null)
              VideoProgressIndicator(
                ...
                colors: const VideoProgressColors(
                  playedColor:
                      Colors.deepPurple, // Color for the played portion
  ...
  void _addMedia() async {
    try {
      FilePickerResult? result = await FilePicker.platform.pickFiles(
        allowMultiple: true,
        type: FileType
            .any, // Change to FileType.media if you want to restrict to media files only
      );
  ...
  void _toggleFullScreen() {
    if (_isFullScreen) {
      ...
      Navigator.push(
        context,
        MaterialPageRoute(
            builder: (_) => FullScreenVideoPlayerScreen(_videoController)),
      ).then((_) {
        setState(() => _isFullScreen = false);
      });
            ...
            return Stack(
              children: track.mediaDurations.asMap().entries.map((entry) {
                final double widthRatio =
                    entry.value.inMilliseconds / oneHourInMilliseconds;
                  ...
                  child: Draggable(
                    ...
                    feedback: Material(
                      child: Container(
                        ...
                        child: Text(_formatDuration(entry.value),
  ...
  Widget _buildMediaItem(
      File file, Duration duration, int trackIndex, int mediaIndex, Key key) {
  ...
  void _onReorder(
      int oldIndex, int newIndex, int trackIndex, MediaTrack track) {
    setState(() {
...
class FullScreenVideoPlayerScreen extends StatelessWidget {
  ...
  const FullScreenVideoPlayerScreen(this.controller, {super.key});
