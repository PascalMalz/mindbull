...
class StartAProgramState extends State<StartAProgram> {
  ...
  void _loadJsonData(String? selectedFile) async {
    try {
      final directory = await getApplicationDocumentsDirectory();
      final files = directory.listSync().whereType<File>().toList();
      final selectedJsonFile =
          files.firstWhere((file) => file.path.endsWith(selectedFile!));
      final String jsonContent = await selectedJsonFile.readAsString();
      final List<dynamic> decodedJson = json.decode(jsonContent);
      setState(() {
        jsonData = decodedJson.cast<Map<String, dynamic>>();
  ...
  Duration calculateTotalDuration(List<Map<String, dynamic>> audioFiles) {
    ...
    for (final data in audioFiles) {
      final int? durationMilliseconds =
          data['durationMilliseconds'] * data['repetition'] as int?;
  ...
  void playAudioFilesConsecutively({seekIndex = 0, seekAbs = 0}) async {
    ...
    if (seekAbs > 0) {
      ...
      for (iteration; iteration < filesInJsonData; iteration++) {
        ...
        for (iterationRepetition;
            iterationRepetition < audioFiles['repetition'];
            iterationRepetition++) {
          calculatedLength =
          ...
          if (calculatedLength >= seekAbs) {
            ...
            seek = Duration(
                milliseconds:
                    (seekAbs - lengthOfPreviousAudioFilesNew).toInt());
            seekIndex = iteration;
    ...
    if (jsonData.isNotEmpty) {
      ...
      for (iteration; iteration < filesInJsonData; iteration++) {
        ...
        for (iterationRepetition;
            iterationRepetition < audioFile['repetition'];
            iterationRepetition++) {
          print(
              'iterationRepetition: $iterationRepetition seekIndex: $seekIndex');
          if (!running || !releasedToPlay) {
            //Needed for the scrollbar functionality
          ...
          if (running) {
            if (mounted) {
              setState(() {
                ...
                currentAudioFileDuration =
                    Duration(seconds: audioFile['duration']);
              });
            }

            final audioSource =
                AudioSource.uri(Uri.parse(audioFile['clientAppAudioFilePath']));
            ...
            if (filesInJsonData > iteration) {
              lengthOfPreviousAudioFiles = lengthOfPreviousAudioFiles +
                  audioFile['durationMilliseconds'];
            }
          }
        }
        repetitionSeek = 0;
  ...
  void pauseAudioFiles() {
    currentSeekPosition = Duration(
        milliseconds: player.position.inMilliseconds +
            lengthOfPreviousAudioFiles.toInt());
...
/*          const SizedBox(height: 16.0),
          ...
          Center(
            child: Container(
              ...
              color:
                  Colors.grey, // Set any background color you want for the box
              child: Image.asset(
                  "assets/fileCardPic.jpg"), // Place the image inside the square box
            ),
          ),
          LayoutBuilder(
              ...
              return StreamBuilder<Duration>(
                ...
                builder: (context, snapshot) {
                  return ProgressBar(
                    ...
                    fileDurations: jsonData
                        .map((audioFiles) =>
                            Duration(seconds: audioFiles['duration']))
                        .toList(),
...
class JsonFileDropdown extends StatefulWidget {
  ...
  const JsonFileDropdown({super.key, required this.onFileSelected});
