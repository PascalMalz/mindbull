...
class _FollowersListScreenState extends State<FollowersListScreen> {
  ...
  Future<void> _fetchFollowers() async {
    ...
    try {
      List<dynamic> followersList = await apiFollowUser.fetchFollowersList(
          widget.userId!); // Using ! because we checked it's not null
      setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: _isLoading
          ...
          : _followers.isEmpty
              ? const Center(child: Text('No following found'))
              : RefreshIndicator(
                      ...
                      return FollowerCard(
                        ...
                        trailing: const Text('') ??
                            IconButton(
                              ...
                              onPressed: () async {
                                await apiFollowUser
                                    .followUser(_followers[index]['id']);
                                setState(
                                    () {}); // Ensure you have access to setState, or use other state management
                              },
                            ),
