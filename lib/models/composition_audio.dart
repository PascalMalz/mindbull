...
class CompositionAudio {
  ...
  factory CompositionAudio.fromJson(Map<String, dynamic> json) {
    ...
    var contentType = json[
        'content_type']; // Assuming 'type' field indicates whether it's an AudioFile or Composition
    print('contentType: $contentType');
    print(
        'CompositionAudio.fromJson: fine until here contentJson: $contentJson');
    dynamic content;
    ...
    var audioPosition = json['audio_position'] != null
        ? int.tryParse(json['audio_position'].toString()) ?? 0
        : 0;
    var audioRepetition = json['audio_repetition'] != null
        ? int.tryParse(json['audio_repetition'].toString()) ?? 0
        : 1;
  ...
  Map<String, dynamic> toJson() {
    Map<String, dynamic>? contentJson; // Make it nullable
    if (content is Audio) {
      contentJson = (content as Audio).toJson();
      contentJson['type'] =
          'audio_file'; // Add a 'type' field to indicate the content type
    } else if (content is Composition) {
      contentJson = (content as Composition).toJson();
      contentJson['type'] =
          'composition'; // Add a 'type' field to indicate the content type
    } else {
      throw Exception('Unsupported content type for serialization');
    }
